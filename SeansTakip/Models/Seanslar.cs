namespace SeansTakip.Models
{
    public partial class Seanslar
    {
        ...
        public static void saveDatas(Seanslar session)
        {
            if (Program.existsDB())
            {
                try
                {
                    ...
                    {
                        SinemaSalon? movieTheater = context.MovieTheaters.FirstOrDefault(s => s.movieTheaterName == session.movieTheaterName);
        ...
        public static void listDatas()
        {
            if (Program.existsDB())
            {
                try
                {
                    using (DBContext context = new DBContext())
                    {
                        List<Seanslar> sessions = context.Sessions.ToList();
            ...
            if (Program.existsDB())
            {
                try
                {
                    using (DBContext context = new DBContext())
                    {
                        ...
                        List<string?> movies = context.MovieName.Select(f => f.movieName).ToList();
        ...
        public static void fillMovieTimes()
        {
            if (Program.existsDB() && Program.homePage.sMovieNameCombo.SelectedItem != null)
            {
                try
                {
                    using (DBContext context = new DBContext())
                    {
                        var movieTimes = context.MovieName.Where(f => f.movieName == Program.homePage.sMovieNameCombo.SelectedItem.ToString()).Select(f => new { f.timeOne, f.timeTwo, f.timeThree, f.timeFour }).FirstOrDefault();
